...
public class MyService extends DefaultService {
	...
	public MyService() {
		...
		fault.setElement(new Element("Description", namespace, SchemaUtil.TYPE_STRING));
	...
	private class GetCurrentTime extends Operation {
		...
		GetCurrentTime() {
			...
			setOutput(new Element(new QName("TimeOutput", namespace), stringType));
		...
		public ParameterValue invokeImpl(ParameterValue parameterValue, CredentialInfo credentialInfo) throws InvocationException, CommunicationException {
			...
			ParameterValueManagement.setString(paramTimeVal, null, MyDevice.getCurrentTime(null));
			return paramTimeVal;
		}
	}
	...
	private class CalcAddition extends Operation {
		...
		public ParameterValue invokeImpl(ParameterValue parameterValue, CredentialInfo credentialInfo) throws InvocationException, CommunicationException {
			...
			try {
				...
				ParameterValueManagement.setString(paramTimeVal, null, String.valueOf(val_a + val_b));
			} catch (NumberFormatException e) {
				ParameterValueManagement.setString(paramTimeVal, null, "<ERROR>");

				ParameterValue result = fault.createValue();
				ParameterValueManagement.setString(result, null, "Invalid Input: Integer values expected.");
				throw new InvocationException(fault, SOAPConstants.SOAP_FAULT_SENDER, result);
			}
